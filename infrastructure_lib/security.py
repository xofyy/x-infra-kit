...
class StandardIdentity(Construct):
    ...
    def __init__(
        ...
        self.gsa = ServiceAccount(
            self, "sa", account_id=sa_id, display_name=f"Workload Identity SA for {sa_id}"
        )
        ...
        self.workload_identity_binding = ServiceAccountIamBinding(
            self,
            "workload_identity_user",
            service_account_id=self.gsa.name,
            role="roles/iam.workloadIdentityUser",
            members=[f"serviceAccount:{project_id}.svc.id.goog[{k8s_namespace}/{k8s_sa_name}]"],
        )
